...
//{
public class TigerDeathMove : MonoBehaviour
{
    ...
    //	public bool PingPongPath=true;
    public PathMovementType PathType = PathMovementType.PingPong;
    ...
    public bool ResetWithoutPlayer = false;
    ...
    bool _OriginalTransformPositionSet = false;
    ...
    protected virtual void Start()
    {
        ...
        if (!OnlyMovesWhenPlayerIsColliding && ResetWithoutPlayer)
        {
            _direction = 1;
        }
        else
    ...
    protected virtual void FixedUpdate()
    {
        ...
        //		if (OnlyMovesWhenPlayerIsColliding && !_collidingWithPlayer && _direction == 1)
        if (OnlyMovesWhenPlayerIsColliding && !ResetWithoutPlayer && !_collidingWithPlayer)
        {
            //			if (_direction == 1)
            //				_direction = -1;
            //			if (
        ...
        if (distanceSquared < MinDistanceToGoal * MinDistanceToGoal)
        {
            ...
            //			if (!_collidingWithPlayer && _direction == 1)
            //				return;
        ...
        if (autoRotate) transform.right = -Vector3.Normalize(CurrentSpeed);
    ...
    public virtual IEnumerator<Vector3> GetPathEnumerator()
    {
        ...
        while (true)
        {
            ...
            if (PathType == PathMovementType.Loop)
            {
            ...
            else if (PathType == PathMovementType.PingPong)
            {
                if (index <= 0)
                {
                    _direction = 1;
            ...
            else
            {
                ...
                index = index + _direction;
                //				index = Mathf.Clamp (index + _direction, 0, Path.Count);
    ...
    protected virtual void OnDrawGizmos()
    {
        ...
        for (int i = 0; i < Path.Count; i++)
        {
            ...
            if ((i == Path.Count - 1) && (PathType == PathMovementType.Loop))
            {
                Gizmos.color = Color.white;
